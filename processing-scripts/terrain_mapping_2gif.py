...
GIF_FOLDER = '/home/andre/Desktop/gif_folder/'
...
for k in range(0,nx):
    ...
    Imagen_aux = np.reshape(Imagen, (nposy, nposx))
    Imagen_aux = np.flipud(Imagen_aux)
    Imagen_aux = np.absolute(Imagen_aux)
    Imagen_aux /= (nx * nfre)

    fig = plt.figure(1)
    im = plt.imshow(Imagen_aux, cmap = 'jet', aspect = 'auto', extent = [xi,xf,yi,yf])
    cbar = plt.colorbar(im, orientation = 'vertical')
    plt.ylabel('Range (m)', fontsize = 14)
    plt.xlabel('Cross-range (m)', fontsize = 14)
    plt.savefig(GIF_FOLDER + 'Posicion_%03d.png' %(k + 1))
    fig.clear()
